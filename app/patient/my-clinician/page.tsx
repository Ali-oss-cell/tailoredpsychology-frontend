  ...
  return (
    <PatientShell activeRoute="my-clinician">
      <section className="space-y-6" data-tutorial="patient.page.my-clinician">
        ...
        {appointmentsError ? (
          <DashboardStateBlock variant="empty" message={appointmentsError} />
        ) : null}
            ...
            ? team.map((clinician) => {
                ...
                const specialtyLine =
                  clinician.specialties.length > 0
                    ? `Focus areas: ${clinician.specialties.join(", ")}`
                    : undefined

                return (
                  <Card
                    key={clinician.clinicianId}
                    className="border-border/70 motion-safe:transition-shadow motion-safe:duration-200 motion-safe:hover:shadow-md focus-within:ring-2 focus-within:ring-ring/50 motion-reduce:transition-none"
                  >
                    <CardHeader className="pb-2">
                    ...
                    <CardContent className="space-y-4 text-sm">
                      ...
                      {appointmentLists ? (
                        <>
                          <div>
                            <p className="text-foreground mb-2 font-medium">Upcoming</p>
                            {upcoming.length === 0 ? (
                              ...
                              <ul className="space-y-2">
                                {upcoming.map((item) => {
                                  const { date, time } = formatSessionRange(
                                  ...
                                  return (
                                    <li
                                      ...
                                      className="border-border/50 bg-muted/35 flex flex-wrap items-center justify-between gap-2 rounded-md border px-2 py-2 text-xs"
                                    >
                          ...
                          <div>
                            <p className="text-foreground mb-2 font-medium">Recent sessions</p>
                            {past.length === 0 ? (
                              ...
                              <ul className="space-y-1.5">
                                {past.slice(0, 5).map((item) => {
                                  ...
                                  return (
                                    <li
                                      key={item.appointmentId}
                                      className="border-border/40 flex flex-wrap items-center justify-between gap-2 border-b py-2 text-xs last:border-b-0"
                                    >
                                      <span>{date}</span>
