...
import {
  getAdminDeletionQueue,
  type AdminDeletionQueueItem,
...
export default function AdminDataDeletionPage() {
  ...
  const [stateFilter, setStateFilter] = React.useState<
    "all" | "deleted" | "legal_hold" | "purge_eligible"
  >("all")
  ...
  const columns: AdminDataTableColumn<AdminDeletionQueueItem>[] = [
    ...
    },
    {
      ...
      sortable: true,
      sortValue: (row) =>
        row.deletedAt ? new Date(row.deletedAt).getTime() : 0,
      renderCell: (row) =>
        row.deletedAt ? formatDateTimeAu(row.deletedAt) : "active",
    },
    {
      key: "retentionUntil",
      ...
      sortable: true,
      sortValue: (row) =>
        row.retentionUntil ? new Date(row.retentionUntil).getTime() : 0,
      renderCell: (row) =>
        row.retentionUntil ? formatDateTimeAu(row.retentionUntil) : "n/a",
    },
    {
      key: "legalHold",
      header: "Legal hold",
    ...
    {
      key: "purgeEligible",
      header: "Purge eligible",
      sortable: true,
      sortValue: (row) => (row.purgeEligible ? 1 : 0),
      renderCell: (row) => (row.purgeEligible ? "yes" : "no"),
    },
  ]
  ...
  return (
    <OpsPortalPage
      ...
      title={opsPagesContent.adminDataDeletion.title}
      description={opsPagesContent.adminDataDeletion.description}
    >
      <AdminFilterBar
        searchValue=""
        ...
        onSelectChange={(_, value) =>
          setStateFilter(
            value as "all" | "deleted" | "legal_hold" | "purge_eligible"
          )
        }
      />
      <AdminDataTable
