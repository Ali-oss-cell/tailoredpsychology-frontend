...
export function PsychologistNotesWorkspace() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!cancelled) setNoteLoadError("Could not load selected note.")
      }
    })()
    return () => {
      cancelled = true
    }
  }, [psychologistId, selectedNoteId, noteRetryNonce])
      ...
      >
        ...
        <Card className={cn("dashboard-card interactive-lift", !showQueuePanel && "hidden md:block")}>
          ...
          <CardContent className="space-y-2">
            ...
            {error ? <DashboardStateBlock variant="error" message={error} onRetry={() => window.location.reload()} /> : null}
            {noteLoadError ? (
              <DashboardStateBlock
                variant="error"
                message={noteLoadError}
                onRetry={() => {
                  setNoteLoadError(null)
                  setNoteRetryNonce((n) => n + 1)
                }}
              />
            ) : null}
